...
- (void) setPointsCoordinates: (CGPoint[]) points {
    ...
    for (uint i = 0; i < 4; i++)
    {
        [p addObject:[NSValue valueWithCGPoint:points[i]]];
    }
    [_pointsView removeFromSuperview];
    _pointsView = [[JBCroppableLayer alloc] initWithImageView:self];
    [_pointsView addPointsAt:p];
    [self addSubview:_pointsView];
}

- (NSArray *) getPoints {
    NSMutableArray *curPoints = [_pointsView.getPoints mutableCopy];
...
- (void)removePoint {
    ...
    NSInteger indexOfSmallestGap = 0;
